...
describe('patchReceipts', () => {
  it('dispatches', async () => {
    expect.assertions(2)
    const updatedReceipt = { value: 'updated' }
    patchReceipt.mockReturnValue(Promise.resolve(updatedReceipt))
    ...
    expect(firstAction.payload).toEqual(updatedReceipt)
  })

  it('dispatches error when api call fails', () => {
    patchReceipt.mockImplementation(() => Promise.reject(new Error('uh oh!')))
    expect.assertions(2)
